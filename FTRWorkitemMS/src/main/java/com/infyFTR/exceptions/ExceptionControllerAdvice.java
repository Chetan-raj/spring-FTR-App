	...
	public ResponseEntity<ErrorMessage> exceptionHandler2(Exception ex) 
 	{
	     ...
	     error.setMessage(environment.getProperty(InfyFTRConstants.GENERAL_EXCEPTION_MESSAGE.toString()));
	     return new ResponseEntity<>(error, HttpStatus.OK);
	}
	
	//Handler for NoSuchCustomerException
	@ExceptionHandler(NoSuchWorkitemException.class)
	public ResponseEntity<ErrorMessage> exceptionHandler2(NoSuchWorkitemException ex) 
	{
		 ErrorMessage error = new ErrorMessage();
